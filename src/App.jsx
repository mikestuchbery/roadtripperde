...
export default function App() {
  ...
  async function findStops() {
    if (!start || !end) return;
    setLoading(true); setPois([]); setHasSearched(true);
    try {
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .hero::before {
          ...
          position: absolute;
          inset: 0;
          background: radial-gradient(ellipse at 30% 0%, rgba(184,130,50,0.22) 0%, transparent 60%),
                      radial-gradient(ellipse at 80% 80%, rgba(120,60,20,0.18) 0%, transparent 50%);
          pointer-events: none;
        }
        .hero-eyebrow {
          font-size: 10px;
        ...
        .scene-sun {
          ...
          box-shadow: 0 0 18px 6px rgba(255,190,40,0.5);
        }
        /* Clouds */
        .scene-cloud {
          position: absolute;
        ...
        /* Trees */
        .scene-treeline {
          position: absolute; bottom: 26px; left: 0; right: 0; height: 44px;
        }
        .scene-tree {
          position: absolute; bottom: 0;
          animation: tree 3.2s linear infinite;
        }
        ...
        .scene-road {
          ...
          background: linear-gradient(180deg, #7A7A7A 0%, #686868 100%);
        ...
        .scene-car {
          position: absolute;
          bottom: 8px; left: 26%;
          animation: bob .32s ease-in-out infinite alternate;
        }
        ...
        .scene-win {
          position: absolute;
          top: -10px; height: 9px;
          background: rgba(160,210,240,0.88);
        ...
        .scene-wheel--front { right: 7px; }
        .scene-wheel--rear  { left: 7px; }
        @keyframes spin { to { transform: rotate(360deg); } }
        .scene-puff {
          position: absolute; border-radius: 50%;
          background: rgba(210,210,210,0.5);
          animation: puff .65s ease-out infinite;
        }
        ...
        @keyframes puff {
          0%   { opacity: .7; transform: scale(.5) translateX(0); }
          100% { opacity: 0;  transform: scale(1.8) translateX(-12px); }
        }
        ...
        .search-explore-btn:active { background: #A03820; transform: scale(.98); }
        ...
        .card {
          ...
          overflow: hidden;
          margin-bottom: 16px;
          box-shadow: 0 2px 12px rgba(30,16,4,.1), 0 1px 3px rgba(30,16,4,.07);
          animation: cardIn .42s ease both;
          /* subtle border */
          outline: 1px solid rgba(180,150,80,.18);
        }

        /* Hero image inside card — full bleed, taller on mobile */
        .card-hero {
          position: relative;
          height: 220px;
          overflow: hidden;
        ...
        .card-hero-fade {
          position: absolute; inset: 0;
          background: linear-gradient(180deg, transparent 40%, rgba(20,10,2,.55) 100%);
        }
        /* Stop number badge — overlaid on image */
        .card-hero-num {
          position: absolute;
          top: 12px; left: 12px;
          width: 30px; height: 30px;
          border-radius: 50%;
          background: rgba(20,10,2,.72);
        ...
        .card-link {
          ...
          letter-spacing: .02em;
          padding: 8px 14px;
          background: rgba(192,72,48,.08);
          border-radius: 6px;
          transition: background .15s;
        }
        .card-link:active { background: rgba(192,72,48,.16); }
        ...
        .footer-rule {
          height: 1px;
          background: linear-gradient(90deg, transparent, #C4A860, transparent);
          margin-bottom: 20px;
        }
        .footer-road {
          font-size: 11px;
        ...
        .footer-email {
          ...
          color: #C04830;
          text-decoration: none;
          border-bottom: 1px solid rgba(192,72,48,.3);
          padding-bottom: 1px;
        }
        .footer-data {
          margin-top: 20px;
        ...
        .kofi:active { transform: scale(.92) rotate(-8deg); }
      ...
      <div className="page">
        ...
        <div className="search-panel">
          ...
          <button className="search-explore-btn" onClick={findStops}>
        ...
        <div className="content">
          ...
          {!loading && shown.length > 0 && (
            <>
              ...
              <div className="maps-banner">
                <p className="maps-banner-label">Open journey in maps</p>
                <div className="maps-btns">
                  <a
                    className="maps-btn maps-btn--google"
                    href={`https://www.google.com/maps/dir/${encodeURIComponent(start)}/${encodeURIComponent(end)}`}
                    target="_blank" rel="noreferrer"
                  >
                    Google Maps →
                  </a>
                  <a
                    className="maps-btn maps-btn--apple"
                    href={`https://maps.apple.com/?saddr=${encodeURIComponent(start)}&daddr=${encodeURIComponent(end)}&dirflg=d`}
                    target="_blank" rel="noreferrer"
                  >
                    Apple Maps →
                  </a>
          ...
          {/* ── FOOTER ── */}
          {(hasSearched || pois.length > 0) && !loading && (
      ...
      <a href="https://buymeacoffee.com/mikestuchbery" target="_blank" rel="noopener noreferrer" className="kofi">
        ☕
      </a>
    </>
  );
